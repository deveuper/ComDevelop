...
namespace ComDevelop.EditorTool.ShaderTool
{
    ...
    public class MeshMakerEditor : Editor
    {
        ...
        private void Init()
        {
            ...
            {
                Texture2D t2d = AssetDatabase.LoadAssetAtPath("Assets/ComDevelop/Editor/MeshMaker/Editor/Texture/Vertex.png", typeof(Texture2D)) as Texture2D;
                _vertexButtonContent = new GUIContent("", t2d, "Vertex Edit Mode");
            }
            if (_edgeButtonContent == null)
            {
                Texture2D t2d = AssetDatabase.LoadAssetAtPath("Assets/ComDevelop/Editor/MeshMaker/Editor/Texture/Edge.png", typeof(Texture2D)) as Texture2D;
                _edgeButtonContent = new GUIContent("", t2d, "Edge Edit Mode");
            }
            if (_faceButtonContent == null)
            {
                Texture2D t2d = AssetDatabase.LoadAssetAtPath("Assets/ComDevelop/Editor/MeshMaker/Editor/Texture/Face.png", typeof(Texture2D)) as Texture2D;
                _faceButtonContent = new GUIContent("", t2d, "Face Edit Mode");
            }
            if (_noneButtonContent == null)
            {
                Texture2D t2d = AssetDatabase.LoadAssetAtPath("Assets/ComDevelop/Editor/MeshMaker/Editor/Texture/None.png", typeof(Texture2D)) as Texture2D;
        ...
        private void SaveMesh()
        {
            ...
            if (path.Length != 0)
            {
                ...
                //2018.1.16修改,保存后放到该物体上
                _meshMaker.transform.GetComponent<MeshFilter>().mesh = _meshMaker._meshFilter.sharedMesh;
            }
            StopSecondaryHandle();
        }
        /// <summary>
